...
typedef struct s_transd_debug {
	uint8_t dist_x; //x position of the center of the transducer in milimeters.
	uint8_t dist_y; //y position of the center of the transducer in milimeters.
	uint8_t dist; //dist between the transducer and the focus in milimeters.
...
/* VARIABLES */
t_transd transd_array[ARRAY_SIZE_X][ARRAY_SIZE_Y];
#ifdef DEBUG
t_transd_debug transd_array_debug[ARRAY_SIZE_X][ARRAY_SIZE_Y];
#endif
...
uint8_t transd_array_set( /*t_transd_array *transd_array,*/ const uint8_t index_x, const uint8_t index_y, const uint8_t port_pin, const uint8_t phase_comp ){
	...
	if(index_x >= ARRAY_SIZE_X) {
		#ifdef DEBUG
		DEBUG_MSG("index_x out of bouds")
		#endif
		return 4;
	}
	...
	if(index_y >= ARRAY_SIZE_Y) {
		#ifdef DEBUG
		DEBUG_MSG("index_y out of bouds")
		#endif
		return 4;
	}
...
uint8_t transd_array_calcfocus( /*t_transd_array *transd_array,*/ const uint8_t duty_cycle, const uint8_t focus_x, const uint8_t focus_y, const uint8_t focus_z ) {
	...
	for(x = 0; x < ARRAY_SIZE_X; x++){
		for(y = 0; y < ARRAY_SIZE_Y; y++){
			...
			dist_x = (TRANS_DIAMETER / 2) + ((TRANS_DIAMETER+TRANS_SEPARATION) * x); //calculates the x position of the transducer on the array
			dist_y = (TRANS_DIAMETER / 2) + ((TRANS_DIAMETER+TRANS_SEPARATION) * y); //calculates the y position of the transducer on the array
			#ifdef DEBUG
			...
			*/
			dist_x = dist_x - focus_x; //distance on the x axys
			dist_y = dist_y - focus_y; //distance on the y axys
...
uint8_t transd_array_calcflat( /*t_transd_array *transd_array,*/ const uint8_t duty_cycle ) {
	...
	for(x = 0; x < ARRAY_SIZE_X; x++){
		for(y = 0; y < ARRAY_SIZE_Y; y++){
			...
			#ifdef DEBUG
			transd_array_debug[x][y].dist_x = ((TRANS_DIAMETER / 2) + ((TRANS_DIAMETER+TRANS_SEPARATION) * x); //calculates the x position of the transducer on the array
			transd_array_debug[x][y].dist_y = ((TRANS_DIAMETER / 2) + ((TRANS_DIAMETER+TRANS_SEPARATION) * y); //calculates the y position of the transducer on the array
			#endif
