...
uint16_t transd_getPattern( const uint8_t phase, const uint8_t duty );

uint8_t transd_init( t_transd *transd, const uint8_t index_x, const uint8_t index_y, const uint8_t port_pin, const uint8_t phase_compn );
uint8_t transd_set( t_transd *transd, uint8_t port_pin, uint8_t phase_comp );
uint8_t transd_set2( t_transd *transd, uint8_t duty, uint8_t phase );
uint8_t transd_calcfocus( t_transd *transd, const uint8_t duty_cycle, const uint8_t focus_x, const uint8_t focus_y, const uint8_t focus_z );
uint8_t transd_calcflat( t_transd *transd, const uint8_t duty_cycle );
...
uint8_t transd_array_init( t_transd *transd_array ) {	
	...
	for(x = 0; x < TRANSD_SIZE_X; x++){
		for(y = 0; y < TRANSD_SIZE_Y; y++){
			
			ret = transd_init( (transd_array + x * TRANSD_SIZE_Y + y), x, y, 0, 0 );
			if(ret != 0) { 
				return ret;
			}
		}
...
uint8_t transd_array_set( t_transd *transd_array, const uint8_t index_x, const uint8_t index_y, const uint8_t port_pin, const uint8_t phase_comp ){
	...
	ret = transd_set( (transd_array + index_x * TRANSD_SIZE_Y + index_y), port_pin, phase_comp );
...
uint8_t transd_array_set2( t_transd *transd_array, const uint8_t index_x, const uint8_t index_y, const uint8_t duty, const uint8_t phase ){
  ...
  ret = transd_set2( (transd_array + index_x * TRANSD_SIZE_Y + index_y), duty, phase );
...
uint8_t transd_array_calcfocus( t_transd *transd_array, const uint8_t duty_cycle, const uint8_t focus_x, const uint8_t focus_y, const uint8_t focus_z ) {
	...
	for(x = 0; x < TRANSD_SIZE_X; x++){
		for(y = 0; y < TRANSD_SIZE_Y; y++){
			...
			ret = transd_calcfocus( (transd_array + x * TRANSD_SIZE_Y + y), duty_cycle, focus_x, focus_y, focus_z );
			if(ret != 0) {
				return ret;
			}
		}
...
uint8_t transd_array_calcflat( t_transd *transd_array, const uint8_t duty_cycle ) {
	...
	for(x = 0; x < TRANSD_SIZE_X; x++){
		for(y = 0; y < TRANSD_SIZE_Y; y++){
			...
			ret = transd_calcflat( (transd_array + x * TRANSD_SIZE_Y + y), duty_cycle );
			if(ret != 0) {
				return ret;
			}
		}
...
uint8_t transd_dumpJSON ( FILE *f,  t_transd *transd_array ){
	...
	sprintf(buff, "{\n\t\"size_x\": %u,\n\t\"size_y\": %u,\n\t\"elem_diameter\": %u,\n\t\"elem_separation\": %u,\n\t\"phase_res\": %u,\n\t\"transd_ptr\": [", TRANSD_SIZE_X, TRANSD_SIZE_Y, TRANSD_DIAM, TRANSD_SEPAR, TRANSD_PHRES);
	fputs(buff, f);
	...
	for(x = 0; x < TRANSD_SIZE_X; x++){
		for(y = 0; y < TRANSD_SIZE_Y; y++){
			...
			transd_ptr = (transd_array + x * TRANSD_SIZE_Y + y);
			...
			for(i = 0; i < 16; i++) {
				...
				if(transd_ptr->pattern & mask) { //if the pattern have the current bit = 1
					...
					patt[TRANSD_PHRES - 1 - i] = '#'; //upgraded to show only the used bits of the pattern
				}
				else {
					//patt[15 - i] = '_';
					patt[TRANSD_PHRES - 1 - i] = '_'; //upgraded to show only the used bits of the pattern
				}
			}
			//patt[16] = '\0';
			patt[TRANSD_PHRES] = '\0'; //upgraded to show only the used bits of the pattern
...
uint16_t transd_getPattern( const uint8_t phase, const uint8_t duty ){
	...
	/*
		...
		50% -> _____#####
	*/
	bits_duty = (((duty * 100) / 255) / TRANSD_PHRES); //equation for duty domain [0,255]
	...
	*/
	bits_phase = (phase * TRANSD_PHRES) / 255; //equation for phase domain [0,255]
	...
	*/
	if(bits_phase + bits_duty > TRANSD_PHRES) { //if if the high bits will overflow outside the bits defined by the resolution
		...
		bits_duty = TRANSD_PHRES - bits_duty;
		bits_phase = bits_duty + bits_phase - TRANSD_PHRES;
		pattern = (1 << bits_duty) - 1; 
		pattern <<= bits_phase;
		pattern = ~pattern;
	}
 ...
 * @return 0 for sucess, an error code otherwise.
 */
uint8_t transd_init( t_transd *transd, const uint8_t index_x, const uint8_t index_y, const uint8_t port_pin, const uint8_t phase_comp ) {
	...
	transd->x = ((TRANSD_DIAM / 2) + (TRANSD_DIAM * index_x) + (TRANSD_SEPAR * index_x)); //calculates the position of the transducer on the array
	transd->y = ((TRANSD_DIAM / 2) + (TRANSD_DIAM * index_y) + (TRANSD_SEPAR * index_y)); //calculates the position of the transducer on the array
	transd->phase = 0;
	transd->duty_cycle = 0;
	transd->pattern = 0;
...
uint8_t transd_set2( t_transd *transd, uint8_t duty, uint8_t phase ) {
  ...
  transd->duty_cycle = duty;
  transd->phase = phase;

  transd->pattern = transd_getPattern( phase, duty ); 
...
uint8_t transd_calcflat( t_transd *transd, const uint8_t duty_cycle ) {
	...
	transd->duty_cycle = duty_cycle;
	transd->pattern = transd_getPattern( transd->phase, transd->duty_cycle ); 
